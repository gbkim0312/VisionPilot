...
TEST_F(VideoLoaderTest, FrameReceptionWithDisplay)
{
    ...
    EXPECT_CALL(mock_receive_port_, onFrameReceived(testing::_))
        .WillRepeatedly(testing::Invoke([](std::shared_ptr<domain::model::ImagePacket> packet)
                                        {
            if (!packet || packet->data.empty()) return;

            // ImagePacket 데이터를 cv::Mat으로 래핑 (복사 없음)
            cv::Mat frame(packet->height, packet->width,
                          packet->channels == 3 ? CV_8UC3 : CV_8UC1,
                          packet->data.data());
