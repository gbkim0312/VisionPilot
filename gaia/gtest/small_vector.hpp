...
#if defined(GCH_HAS_CPP20_CONSTEXPR) && defined(GCH_LIB_IS_CONSTANT_EVALUATED) && defined(GCH_LIB_CONSTEXPR_MEMORY)
...
concept ConvertibleTo =
    std::is_convertible<From, To>::value && requires(typename std::add_rvalue_reference<From>::type (&f)(void)) {
        static_cast<To>(f());
    };
...
concept DefaultConstructible =
    ConstructibleFrom<T> && requires { T{}; } && requires { ::new (static_cast<void *>(nullptr)) T; };
...
concept CopyConstructible =
    MoveConstructible<T> && ConstructibleFrom<T, T &> && ConvertibleTo<T &, T> && ConstructibleFrom<T, const T &> && ConvertibleTo<const T &, T> && ConstructibleFrom<T, const T> && ConvertibleTo<const T, T>;

template <typename T>
concept NoThrowCopyConstructible =
    NoThrowMoveConstructible<T> && NoThrowConstructibleFrom<T, T &> && NoThrowConvertibleTo<T &, T> && NoThrowConstructibleFrom<T, const T &> && NoThrowConvertibleTo<const T &, T> && NoThrowConstructibleFrom<T, const T> && NoThrowConvertibleTo<const T, T>;
...
concept EmplaceConstructible =
    std::same_as<typename X::value_type, T> && ((requires { typename X::allocator_type; }    // only perform this check if X is
                                                ...
                                                (
                                                    !requires { typename X::allocator_type; } && requires(T *p, Args &&...args) {
#if __cplusplus >= 202002L // c++20 fully featured
                                                        {
                                                            std::construct_at(p, std::forward<Args>(args)...)
                                                        } -> std::same_as<T *>;
...
template <typename T, typename X,
          typename A = typename std::conditional<requires { typename X::allocator_type; },
                                                 ...
                                                 std::allocator<T>>::type>
concept Erasable =
    std::same_as<typename X::value_type, T> && ((requires { typename X::allocator_type; } // if X is allocator aware
                                                 && std::same_as<typename X::allocator_type,
                                                                 typename std::allocator_traits<A>::template rebind_alloc<T>> &&
                                                 (requires(A m, T *p) {
                                                     m.destroy(p);
...
concept NullablePointer =
    EqualityComparable<T> && DefaultConstructible<T> && CopyConstructible<T> && CopyAssignable<T> && Destructible<T> && ConstructibleFrom<T, std::nullptr_t> && ConvertibleTo<std::nullptr_t, T> && requires(T p, T q, std::nullptr_t np) {
        {
            T(np)
        } -> std::same_as<T>;
        {
...
concept AllocatorFor =
    NoThrowCopyConstructible<A> && requires(A a, typename std::allocator_traits<A>::template rebind_alloc<U> b, U xp, typename std::allocator_traits<A>::pointer p, typename std::allocator_traits<A>::const_pointer cp, typename std::allocator_traits<A>::void_pointer vp, typename std::allocator_traits<A>::const_void_pointer cvp, typename std::allocator_traits<A>::value_type &r, typename std::allocator_traits<A>::size_type n) {
        ...
        requires !requires { a.allocate(n, cvp); } || requires { { a.allocate (n, cvp) } -> std::same_as<decltype (p)>; };
        ...
        // a.max_size () [optional]
        requires !requires { a.max_size(); } || requires { { a.max_size () } -> std::same_as<decltype (n)>; };

        // a.construct (xp, args) [optional]
        requires !requires { a.construct(xp); } || requires { { a.construct (xp) } -> std::convertible_to<void>; };

        // a.destroy (xp) [optional]
        requires !requires { a.destroy(xp); } || requires { { a.destroy (xp) } -> std::convertible_to<void>; };
...
template <typename Allocator>
GCH_INLINE_VARIABLE constexpr unsigned
...
public:
    ...
    constexpr small_vector_iterator
...
constexpr auto
operator<=>(const small_vector_iterator<PointerLHS, DifferenceTypeLHS> &lhs,
            const small_vector_iterator<PointerRHS, DifferenceTypeRHS> &rhs) noexcept(noexcept(lhs.base() < rhs.base()) && noexcept(rhs.base() < lhs.base()))
{
...
constexpr auto
operator<=>(const small_vector_iterator<Pointer, DifferenceType> &lhs,
            const small_vector_iterator<Pointer, DifferenceType> &rhs) noexcept(noexcept(lhs.base() < rhs.base()) && noexcept(rhs.base() < lhs.base()))
{
    using ordering = std::weak_ordering;
    return (lhs.base() < rhs.base())   ? ordering::less
           : (rhs.base() < lhs.base()) ? ordering::greater
...
template <typename Pointer, typename DifferenceType>
constexpr small_vector_iterator<Pointer, DifferenceType>
...
public:
    ...
    GCH_NODISCARD constexpr const value_ty *
    ...
    : private Allocator
{
    ...
    GCH_CPP20_CONSTEXPR void
    maybe_assign(const allocator_inliner &other) noexcept(noexcept(Allocator::operator=(other)))
    {
    ...
    template <bool IsNoOp = move_assign_is_noop,
              typename std::enable_if<!IsNoOp, bool>::type = false>
    GCH_CPP20_CONSTEXPR void
    maybe_assign(allocator_inliner &&other) noexcept(noexcept(Allocator::operator=(std::move(other))))
    {
...
public:
    ...
    GCH_CPP20_CONSTEXPR
    allocator_inliner &
    operator=(const allocator_inliner &other) noexcept(noexcept(std::declval<allocator_inliner &>().maybe_assign(other)))
    {
        assert(&other != this && "`allocator_inliner` should not participate in self-copy-assignment.");
        maybe_assign(other);
        return *this;
    }
    ...
    GCH_CPP20_CONSTEXPR
    allocator_inliner &
    operator=(allocator_inliner &&other) noexcept(noexcept(std::declval<allocator_inliner &>().maybe_assign(std::move(other))))
    {
        assert(&other != this && "`allocator_inliner` should not participate in self-move-assignment.");
...
class allocator_inliner<Allocator, false>
{
    ...
    template <bool IsNoOp = copy_assign_is_noop,
              typename std::enable_if<!IsNoOp, bool>::type = false>
    GCH_CPP20_CONSTEXPR void
    maybe_assign(const allocator_inliner &other) noexcept(noexcept(std::declval<decltype(other.m_alloc) &>() = other.m_alloc))
    {
    ...
    template <bool IsNoOp = move_assign_is_noop,
              typename std::enable_if<!IsNoOp, bool>::type = false>
    GCH_CPP20_CONSTEXPR void
    maybe_assign(allocator_inliner &&other) noexcept(noexcept(std::declval<decltype(other.m_alloc) &>() = std::move(other.m_alloc)))
    {
...
private:
    ...
    struct must_use_alloc_construct
        : bool_constant<!std::is_same<A, std::allocator<V>>::value && has_alloc_construct<A, V, Args...>::value>
    {
    };
    ...
    struct must_use_alloc_destroy
        : bool_constant<!std::is_same<A, std::allocator<V>>::value && has_alloc_destroy<A, V>::value>
    {
    };
...
public:
                                  ...
                                  typename std::enable_if<is_complete<From>::value>::type>
    {
        ...
        static constexpr bool
            value = (sizeof(from) == sizeof(to)) && (std::is_same<bool, from>::value == std::is_same<bool, to>::value) && std::is_integral<from>::value && std::is_integral<to>::value;
    };
                         ...
                         typename std::enable_if<is_complete<QualifiedFrom>::value>::type>
    {
        ...
        static constexpr bool
            value = std::is_trivially_assignable<QualifiedTo &, QualifiedFrom>::value && std::is_trivially_copyable<to>::value && (std::is_same<typename std::remove_cv<from>::type, to>::value || is_memcpyable_integral<from, to>::value || is_convertible_pointer<from, to>::value);
    };
    ...
    struct is_uninitialized_memcpyable_impl
    {
        ...
        static constexpr bool
            value = std::is_trivially_constructible<QualifiedTo, QualifiedFrom>::value && std::is_trivially_copyable<to>::value && (std::is_same<typename std::remove_cv<from>::type, to>::value || is_memcpyable_integral<from, to>::value || is_convertible_pointer<from, to>::value) && (!must_use_alloc_construct<alloc_ty, value_ty, from>::value && !must_use_alloc_destroy<alloc_ty, value_ty>::value);
    };
    ...
    struct is_contiguous_iterator
        : bool_constant<
              std::is_same<InputIt, ptr>::value || std::is_same<InputIt, cptr>::value || is_small_vector_iterator<InputIt>::value
#ifdef GCH_LIB_CONCEPTS
              || std::contiguous_iterator<InputIt>
#endif
#ifdef GCH_STDLIB_INTEROP
              || std::is_same<InputIt, typename std::array<value_ty>::iterator>::value || std::is_same<InputIt, typename std::array<value_ty>::const_iterator>::value || (!std::is_same<value_ty, bool> && (std::is_same<InputIt, typename std::vector<value_ty>::iterator>::value || std::is_same<InputIt, typename std::vector<value_ty>::const_iterator>::value)) || std::is_same<InputIt, decltype(std::begin(std::declval<std::valarray<value_ty> &>()))>::value || std::is_same<InputIt, decltype(std::begin(std::declval<const std::valarray<value_ty> &>()))>::value
#endif
              >
    {
    };
    ...
    GCH_NORETURN
    static GCH_CPP20_CONSTEXPR void
    ...
    static constexpr const value_ty *
    ...
    GCH_NODISCARD static GCH_CONSTEVAL
        std::size_t
    ...
    template <typename Iterator,
              typename IteratorDiffT = typename std::iterator_traits<Iterator>::difference_type,
              typename Integer = IteratorDiffT>
    static GCH_CPP17_CONSTEXPR void
    ...
    GCH_CPP20_CONSTEXPR void
    construct(ptr p, Args &&...args) noexcept(noexcept(alloc_traits::construct(std::declval<alloc_ty &>(),
    ...
    GCH_CPP20_CONSTEXPR void
    construct(ptr p, Args &&...args) noexcept(noexcept(::new(std::declval<void *>()) value_ty(std::declval<Args>()...)))
    {
...
private:
    ...
    static GCH_CPP20_CONSTEXPR auto
    construct_at(value_ty *p, Args &&...args) noexcept(noexcept(::new(std::declval<void *>()) V(std::declval<Args>()...)))
...
protected:
    ...
    struct is_explicitly_copy_insertable
        : std::integral_constant<bool, is_emplace_constructible<V &>::value && is_emplace_constructible<const V &>::value>
    {
    };

    template <typename V = value_ty>
    struct is_explicitly_nothrow_copy_insertable
        : std::integral_constant<bool, is_nothrow_emplace_constructible<V &>::value && is_nothrow_emplace_constructible<const V &>::value>
    {
    };
    ...
    struct allocations_are_movable
        : bool_constant<std::is_same<std::allocator<value_ty>, A>::value || std::allocator_traits<A>::propagate_on_container_move_assignment::value
    ...
    struct allocations_are_swappable
        : bool_constant<std::is_same<std::allocator<value_ty>, A>::value || std::allocator_traits<A>::propagate_on_container_swap::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
                        || std::allocator_traits<A>::is_always_equal::value
#endif
                        >
...
private:
    ...
    public:
        ...
        GCH_NODISCARD GCH_CPP20_CONSTEXPR const value_ty &
...
protected:
    ...
    GCH_CPP20_CONSTEXPR
        small_vector_base &
        move_assign_default(small_vector_base<Allocator, LessEqualI> &&other) noexcept(std::is_nothrow_move_assignable<value_ty>::value
        ...
        move_assign_default(small_vector_base<Allocator, GreaterI> &&other)
    {
        if (other.has_allocation())
            move_allocation_pointer(std::move(other));
        else if (get_capacity() < other.get_size() || (has_allocation() && !(other.allocator_ref() == allocator_ref())))
        {
            // Reallocate.
    ...
    template <unsigned LessEqualI,
              typename std::enable_if<(LessEqualI <= InlineCapacity)>::type * = nullptr>
    GCH_CPP20_CONSTEXPR void
    move_initialize(small_vector_base<Allocator, LessEqualI> &&other) noexcept(std::is_nothrow_move_constructible<value_ty>::value)
    {
        if (InlineCapacity < other.get_capacity())
        {
            set_data(other.data_ptr(), other.get_capacity(), other.get_size());
...
public:
    ...
    template <unsigned I>
    GCH_CPP20_CONSTEXPR
    small_vector_base(bypass_tag, small_vector_base<Allocator, I> &&other) noexcept(std::is_nothrow_move_constructible<value_ty>::value || (I == 0 && I == InlineCapacity))
    ...
    template <unsigned I, typename A = alloc_ty,
              typename std::enable_if<std::is_same<std::allocator<value_ty>, A>::value
                                      ...
                                      >::type * = nullptr>
    GCH_CPP20_CONSTEXPR
    small_vector_base(bypass_tag, small_vector_base<Allocator, I> &&other, const alloc_ty &) noexcept(noexcept(small_vector_base(bypass, std::move(other))))
...
protected:
    ...
    template <typename Policy = void, typename V = value_ty,
              typename std::enable_if<is_explicitly_move_insertable<V>::value && (!std::is_same<Policy, strong_exception_policy>::value || relocate_with_move<V>::value),
                                      bool>::type = true>
    GCH_CPP20_CONSTEXPR
        ptr
        uninitialized_move(ptr first, ptr last, ptr d_first) noexcept(std::is_nothrow_move_constructible<value_ty>::value)
    {
    ...
    GCH_CPP20_CONSTEXPR
    void
    swap_elements(small_vector_base &other) noexcept(std::is_nothrow_move_constructible<value_ty>::value
    ...
    GCH_CPP20_CONSTEXPR
    void
    swap_default(small_vector_base &other) noexcept(std::is_nothrow_move_constructible<value_ty>::value
#ifdef GCH_LIB_IS_SWAPPABLE
                                                        &&std::is_nothrow_swappable<value_ty>::value
#else
                                                           &&detail::small_vector_adl::is_nothrow_swappable<value_ty>::value
    ...
    template <typename A = alloc_ty,
              typename std::enable_if<allocations_are_swappable<A>::value && InlineCapacity == 0>::type * = nullptr>
    GCH_CPP20_CONSTEXPR void
    ...
    template <typename A = alloc_ty,
              typename std::enable_if<allocations_are_swappable<A>::value && InlineCapacity != 0>::type * = nullptr>
    GCH_CPP20_CONSTEXPR void
    swap(small_vector_base &other) noexcept(std::is_nothrow_move_constructible<value_ty>::value
#ifdef GCH_LIB_IS_SWAPPABLE
                                                &&std::is_nothrow_swappable<value_ty>::value
#else
                                                   &&detail::small_vector_adl::is_nothrow_swappable<value_ty>::value
        ...
        copy_range(InputIt first, InputIt last, ptr dest)
    {
        ...
        if (std::is_constant_evaluated() && !std::is_same<decltype(unmove_iterator(std::declval<InputIt>())),
                                                          InputIt>::value)
        {
    ...
    template <typename RandomIt,
              typename std::enable_if<
                  !is_memcpyable_iterator<RandomIt>::value && std::is_base_of<std::random_access_iterator_tag,
    ...
    {
#if defined(GCH_LIB_IS_CONSTANT_EVALUATED) && defined(__GLIBCXX__)
        if (std::is_constant_evaluated() && !std::is_same<decltype(unmove_iterator(std::declval<RandomIt>())),
                                                          RandomIt>::value)
        {
    ...
    template <typename InputIt,
              typename std::enable_if<
                  !is_memcpyable_iterator<InputIt>::value && !std::is_base_of<std::random_access_iterator_tag,
...
public:
    ...
    GCH_NODISCARD constexpr cptr
    data_ptr(void) const noexcept
    {
        return m_data.data_ptr();
    }

    GCH_NODISCARD constexpr size_ty
    ...
    GCH_NODISCARD constexpr alloc_ty
...
public:
    ...
    GCH_CPP20_CONSTEXPR explicit small_vector(size_type count, const allocator_type &alloc = allocator_type())
#ifdef GCH_LIB_CONCEPTS
    ...
    template <unsigned I>
        ...
        requires MoveInsertable
#endif
    GCH_CPP20_CONSTEXPR explicit small_vector(small_vector<T, I, Allocator> &&other) noexcept(std::is_nothrow_move_constructible<value_type>::value &&I < InlineCapacity)
        : base(base::bypass, std::move(other))
    {
    }
    ...
    GCH_CPP20_CONSTEXPR
    small_vector &
    operator=(small_vector &&other) noexcept((std::is_same<std::allocator<value_type>, Allocator>::value || std::allocator_traits<Allocator>::propagate_on_container_move_assignment::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
    ...
    GCH_CPP20_CONSTEXPR
    void
    assign(small_vector &&other) noexcept((std::is_same<std::allocator<value_type>, Allocator>::value || std::allocator_traits<Allocator>::propagate_on_container_move_assignment::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
                                           || std::allocator_traits<Allocator>::is_always_equal::value
#endif
                                           ) &&
...
#endif
    GCH_CPP20_CONSTEXPR void
    assign(small_vector<T, I, Allocator> &&other) noexcept(I <= InlineCapacity && (std::is_same<std::allocator<value_type>, Allocator>::value || std::allocator_traits<Allocator>::propagate_on_container_move_assignment::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
                                                                                   || std::allocator_traits<Allocator>::is_always_equal::value
#endif
                                                                                   ) &&
    ...
    template <typename ValueType = value_type,
              typename std::enable_if<
                  (std::is_move_constructible<ValueType>::value && std::is_move_assignable<ValueType>::value
#ifdef GCH_LIB_IS_SWAPPABLE
                   && std::is_swappable<ValueType>::value
#endif
                   ) ||
                  ((std::is_same<std::allocator<value_type>, Allocator>::value || std::allocator_traits<Allocator>::propagate_on_container_swap::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
                    || std::allocator_traits<Allocator>::is_always_equal::value
#endif
                    ) &&
...
#endif
    GCH_CPP20_CONSTEXPR void
    swap(small_vector &other) noexcept((std::is_same<std::allocator<value_type>, Allocator>::value || std::allocator_traits<Allocator>::propagate_on_container_swap::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
                                        || std::allocator_traits<Allocator>::is_always_equal::value
#endif
                                        ) &&
                                       ((std::is_nothrow_move_constructible<value_type>::value && std::is_nothrow_move_assignable<value_type>::value
                                        ...
                                        InlineCapacity == 0))
#ifdef GCH_LIB_CONCEPTS
        requires(MoveInsertable && MoveAssignable && Swappable) || ((std::is_same<std::allocator<value_type>, Allocator>::value || std::allocator_traits<Allocator>::propagate_on_container_swap::value
#ifdef GCH_LIB_IS_ALWAYS_EQUAL
                                                                     || std::allocator_traits<Allocator>::is_always_equal::value
#endif
                                                                     ) &&
...
inline GCH_CPP20_CONSTEXPR bool
...
#endif
          >
inline GCH_CPP20_CONSTEXPR void
