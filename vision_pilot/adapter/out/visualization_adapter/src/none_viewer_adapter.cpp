...
void NoneViewerAdapter::render(const domain::model::Pose &pose, const domain::model::DetectionResult &detections, const domain::model::TrackingResult &tracking, const domain::model::ImagePacket &frame)
{
    LOG_TRA("");

    LOG_TRA("Received frame for rendering in None Viewer. Pose is_lost: {}, Detections count: {}, Tracking count: {}, Frame id: {}", pose.is_lost, detections.detections.size(), tracking.objects.size(), frame.frame_id);
    ...
    std::string tracking_info;
    for (const auto &object : tracking.objects)
    {
        tracking_info +=
            "Class: " + domain::model::ClassIdHelper::toString(object.class_id) +
            ", Confidence: " + std::to_string(object.confidence) +
            ", BBox: [" + std::to_string(object.bbox.x) + ", " +
            std::to_string(object.bbox.y) + ", " +
            std::to_string(object.bbox.width) + ", " +
            std::to_string(object.bbox.height) + "]\n";
    }
